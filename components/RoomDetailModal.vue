<template>
    <div class="overflow-y-auto overflow-x-hidden fixed top-0 right-0 z-50 w-full h-full rounded-lg max-w-sm min-w-sm shadow bg-grayPrimary border border-gray-50/10 min-h-screen max-h-screen p-6" v-if="showModal">
        ...
        <template v-else>
            ...
            <div class="main">
                ...
                <div class="form-group flex justify-between items-center my-5">
                    <input type="text" class="text-white border border-gray-600 p-3 w-full bg-transparent rounded rounded-tr-none rounded-br-none focus:ring-0 focus:border-gray-600" v-model="formData.roomName">
                    <button class="bg-blueLight text-white rounded p-3 border border-blueLight rounded-tl-none rounded-bl-none hover:bg-blueDark hover:border-blueDark" @click="updateRoomDetails">
                ...
                <div class="users mt-3">
                    ...
                    <div class="user mb-4" v-for="user in formData.users" :key="formData.id">
                        <p class="text-gray-300 text-md mb-1">{{ user.username }} <span>{{ user._id == currentUserId ? '(You)' : '' }}</span></p>
